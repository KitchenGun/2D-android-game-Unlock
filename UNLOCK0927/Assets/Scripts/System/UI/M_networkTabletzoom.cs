...
public class M_networkTabletzoom : MonoBehaviour
{
    public bool Tableton = false; // 타블렛이 확대되었습니다.
    public GameObject tablet_Network_UI;
    ...
    public void tabletzoomin() // 타블렛을 눌렀음
    {
        if (Tableton == false)
        {
            ...
            //2080 / 1170
            Tableton = true;
            tablet_Network_UI.SetActive(true); // UI 액티브
        ...
        if(Tableton == true) // 타블렛 확대상태
        {
            if (Input.GetKey(KeyCode.Escape)) // 뒤로키눌름
            {
                ...
                this.transform.localScale = new Vector2(1240, 790);
                    tablet_Network_UI.SetActive(false);
