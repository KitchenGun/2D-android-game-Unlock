...
public class M_LockpickTabletzoom : MonoBehaviour
{
    public bool Tableton = false; // 타블렛이 확대되었습니다.
    public GameObject tablet_Lockpick_UI;
    ...
    public void tabletzoomin() // 타블렛을 눌렀음
    {
        if (Tableton == false)
        {
            ...
            //2080 / 1170
            Tableton = true;
            tablet_Lockpick_UI.SetActive(true); // UI 액티브
        ...
        if(Tableton == true) // 타블렛 확대상태
        {
            if (Input.GetKey(KeyCode.Backspace)) // 뒤로키눌름
            {
                ...
                this.transform.localScale = new Vector2(1240, 790);
                    tablet_Lockpick_UI.SetActive(false);
